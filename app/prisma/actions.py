...
class NoteActions(Generic[_PrismaModelT]):
    ...
    async def group_by(
        ...
        *,
        ...
        -------
        ```py
        # group Note records by id values
...
class NotebookActions(Generic[_PrismaModelT]):
    __slots__ = (
        '_client',
        '_model',
    )
    ...
    async def find_unique_or_raise(
        ...
        """Find a unique Notebook record. Raises `RecordNotFoundError` if no record is found.
        ...
        -------
        ```py
        notebook = await Notebook.prisma().find_unique_or_raise(
            where={
                'id': 1388290519,
            },
        )
    ...
    async def find_many(
        ...
        distinct: Optional[List[types.NotebookScalarFieldKeys]] = None,
    ) -> List[_PrismaModelT]:
        ...
        distinct
            Filter Notebook records by either a single distinct field or distinct combinations of fields
    ...
    async def find_first_or_raise(
        ...
        """Find a single Notebook record. Raises `RecordNotFoundError` if no record was found.
        ...
        -------
        ```py
        # find the second Notebook record ordered by the created_at field
        notebook = await Notebook.prisma().find_first_or_raise(
            skip=1,
            order={
                'created_at': 'desc',
            },
    ...
    async def group_by(
        ...
        *,
        ...
        -------
        ```py
        # group Notebook records by created_at values
        # and count how many records are in each group
